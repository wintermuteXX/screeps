...
(function () {
  ...
  const BASE_MINERALS = [RESOURCE_ENERGY, RESOURCE_KEANIUM, RESOURCE_HYDROGEN, RESOURCE_OXYGEN, RESOURCE_CATALYST, RESOURCE_ZYNTHIUM, RESOURCE_UTRIUM, RESOURCE_LEMERGIUM];
  const BASE_MINERALS_WITHOUT_ENERGY = [RESOURCE_KEANIUM, RESOURCE_HYDROGEN, RESOURCE_OXYGEN, RESOURCE_CATALYST, RESOURCE_ZYNTHIUM, RESOURCE_UTRIUM, RESOURCE_LEMERGIUM];
  const BASE_COMPOUNDS = [RESOURCE_HYDROXIDE, RESOURCE_ZYNTHIUM_KEANITE, RESOURCE_UTRIUM_LEMERGITE, RESOURCE_GHODIUM];
  const TIER_1_COMPOUNDS = [
  ...
  const COMMODITIES_BASIC = [RESOURCE_METAL, RESOURCE_BIOMASS, RESOURCE_SILICON, RESOURCE_MIST];
  const COMMODITIES_LEVEL_0 = [RESOURCE_ALLOY, RESOURCE_TISSUE, RESOURCE_WIRE, RESOURCE_CONDENSATE];
  const COMMODITIES_LEVEL_1 = [RESOURCE_SWITCH, RESOURCE_PHLEGM, RESOURCE_TUBE, RESOURCE_CONCENTRATE, RESOURCE_COMPOSITE];
  const COMMODITIES_LEVEL_2 = [RESOURCE_TRANSISTOR, RESOURCE_TISSUE, RESOURCE_FIXTURES, RESOURCE_EXTRACT, RESOURCE_CRYSTAL];
  const COMMODITIES_LEVEL_3 = [RESOURCE_MICROCHIP, RESOURCE_MUSCLE, RESOURCE_FRAME, RESOURCE_SPIRIT, RESOURCE_LIQUID];
  const COMMODITIES_LEVEL_4 = [RESOURCE_CIRCUIT, RESOURCE_ORGANOID, RESOURCE_HYDRAULICS, RESOURCE_EMANATION];
  const COMMODITIES_LEVEL_5 = [RESOURCE_DEVICE, RESOURCE_ORGANISM, RESOURCE_MACHINE, RESOURCE_ESSENCE];
  ...
  const COMMODITIES_SETS = {
    ...
    metal: [RESOURCE_ALLOY, RESOURCE_TUBE, RESOURCE_FIXTURES, RESOURCE_FRAME, RESOURCE_HYDRAULICS, RESOURCE_MACHINE],
    biomass: [RESOURCE_TISSUE, RESOURCE_PHLEGM, RESOURCE_TISSUE, RESOURCE_MUSCLE, RESOURCE_ORGANOID, RESOURCE_ORGANISM],
   ...
   */
  function calcCommodityRawComponents(
    commodity,
    settings = {
      amount: 1,
      stopArr: [],
    ...
    if (!commodity || !COMMODITIES[commodity] || settings.amount <= 0 || !Array.isArray(settings.stopArr) || settings.maxIteration <= 0) return ERR_INVALID_ARGS;
    // Check to avoid infinite loop.
    for (let i = 0; i < COMPRESSED_RESOURCES.length; i++)
      if (settings.stopArr.indexOf(COMPRESSED_RESOURCES[i]) === -1 && settings.stopArr.indexOf(BASE_MINERALS[i]) === -1) settings.stopArr.push(BASE_MINERALS[i]);
   ...
   * @param {Number}   [settings.returnBasis = 0]          0 BASE_MINERALS | 1 COMPRESSED_RESOURCES | 2 MIXED(stop at both BASIC and COMPRESSED).
   * @returns {CommodityInformation|Number} Object, Information about commodity | ERR_INVALID_ARGS | ERR_MAX_ITERATION_REACHED
   */
  function calcCommodityComponents(
    commodity,
    settings = {
      amount: 1,
      stopArr: [],
    ...
    if (settings.returnBasis === 0) basedResources = basedResources.concat(BASE_MINERALS);
    else if (settings.returnBasis === 1) basedResources = basedResources.concat(COMPRESSED_RESOURCES);
    else if (settings.returnBasis === 2) basedResources = basedResources.concat(BASE_MINERALS, COMPRESSED_RESOURCES);
    ...
    while (queue.length && cnt < settings.maxIteration) {
      ...
      const _ret = calcCommodityRawComponents(resourceType, {
        amount,
        stopArr: stopArr,
      ...
      for (let i = 0; i < settings.stopInfo.length; i++) {
        const type = settings.stopInfo[i][0],
          existingAmount = settings.stopInfo[i][1];
  ...
  function calcExpectedRevenue(
    ...
    const basis = calcCommodityComponents(commodity, {
      amount: settings.amount,
      stopInfo: settings.stopResources,
      stopArr: settings.stopArr,
    });
    ...
    return {
      revenue: _getPrice(commodity) * settings.amount - cost,
      cooldown: basis.cooldown,
   ...
   * @returns {CommodityInformation|Number} Object, Information about commodity | ERR_INVALID_ARGS | ERR_MAX_ITERATION_REACHED
   */
  function calcCommodityComponentsBasedOnExistence(
    commodity,
    settings = {
      amount: 1,
      stopArr: [],
    ...
    return calcCommodityComponents(commodity, {
      amount: settings.amount,
      stopInfo: existingInfo,
      stopArr: settings.stopArr,
    });
  }
  /**
   ...
   */
  function calcBestResponseCommodityBasedOnExistence(
    stopResources = [],
    settings = {
      series: ["biomass", "mist", "metal", "silicon", "compressed", "basic"],
      stopArr: [],
      detectLevel: false,
      perTick: false,
      existenceBased: false,
    ...
    for (const serie of settings.series) {
      ...
      for (const commodity of potentialCommodities) {
        ...
        const expectedRevenue = calcExpectedRevenue(commodity, {
          amount: 1,
          stopArr: settings.stopArr,
          stopResources: availableResourcesInformation,
          ignoreNoData: false,
          existenceBased: false,
          buy: true,
        });
        ...
        if (
          (!settings.perTick && expectedRevenue.revenue > bestCommodity.revenue) ||
