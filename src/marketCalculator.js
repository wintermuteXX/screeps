...
(function () {
    ...
    const BASIC_RESOURCES_WITHOUT_ENERGY = [RESOURCE_KEANIUM, RESOURCE_HYDROGEN, RESOURCE_OXYGEN,
        RESOURCE_CATALYST, RESOURCE_ZYNTHIUM, RESOURCE_UTRIUM, RESOURCE_LEMERGIUM,
        RESOURCE_GHODIUM,
    ];
     ...
     */
    function calcCommodityRawComponents(commodity, settings = {
        amount: 1,
        stopArr: [],
        maxIteration: 1024,
        reverse: true
        ...
        for (let i = 0; i < COMPRESSED_RESOURCES.length; i++)
            if (settings.stopArr.indexOf(COMPRESSED_RESOURCES[i]) === -1 && settings.stopArr.indexOf(BASIC_RESOURCES[i]) === -1) settings.stopArr.push(BASIC_RESOURCES[i]);

        const queue = [
            [commodity, settings.amount]
        ];
        const ret = {
            cooldown: 0,
            components: {},
     ...
     * @returns {CommodityInformation|Number} Object, Information about commodity | ERR_INVALID_ARGS | ERR_MAX_ITERATION_REACHED
     */
    function calcCommodityComponents(commodity, settings = {
        amount: 1,
        stopArr: [],
        stopInfo: [],
        maxIteration: 1024,
    ...
    function _getResourcesInfo(stopResources = [], existenceBased = false, roomNames = [], storeObjects = [], findObjects = false) {
        ...
        const addToListByRoom = function (roomName) {
            ...
            for (let structureName of STORING_SINGLE_OBJECTS)
                if (Game.rooms[roomName][structureName]) addToListByObject(Game.rooms[roomName][structureName]);
            for (let structuresName of STORING_MULTI_OBJECTS)
                if (Game.rooms[roomName][structuresName + "s"])
                    for (let structure of Game.rooms[roomName][structuresName + "s"]) addToListByObject(structure);
        }
        ...
        for (let storeObject of storeObjects)
     ...
     */
    function calcExpectedRevenue(commodity, settings = {
        amount: 1,
        stopArr: [],
        stopResources: [],
        ignoreNoData: true,
     ...
     * @returns {CommodityInformation|Number} Object, Information about commodity | ERR_INVALID_ARGS | ERR_MAX_ITERATION_REACHED
     */
    function calcCommodityComponentsBasedOnExistence(commodity, settings = {
        amount: 1,
        stopArr: [],
        stopResources: [],
        existenceBased: false,
     ...
     */
    function calcBestResponseCommodityBasedOnExistence(stopResources = [], settings = {
        ...
        if (!settings.detectLevel) factoryLevel = [0, 1, 2, 3, 4, 5];
            ...
            let factoryLevels = _.map(_.filter(Game.rooms, (r) => r.controller && r.controller.my), (r) => r.find(FIND_STRUCTURES, {
                filter: {
                    structureType: STRUCTURE_FACTORY
                }
            })[0]).map(f => f && f.level).filter(l => l);
    ...
    const MC = {
        BASIC_RESOURCES,
        BASIC_RESOURCES_WITHOUT_ENERGY,
