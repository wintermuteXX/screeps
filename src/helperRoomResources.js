...
global.RES_TREE = {
    ...
    "基础资源": {
        "能量": ["energy", "battery"],
        "原矿": ["U", "L", "K", "Z", "X", "O", "H", "G"],
        "压缩": ["utrium_bar", "lemergium_bar", "keanium_bar", "zynthium_bar", "purifier", "oxidant", "reductant", "ghodium_melt"]
    },
    "商品资源": {
        "无色": ["liquid", "crystal", "composite"],
...
let pro = {
    ...
    addStore: (store, b) => {
        for (let v in b)
            if (b[v] > 0) store[v] = (store[v] || 0) + b[v];
        return store
    },
    showAllRes() {
